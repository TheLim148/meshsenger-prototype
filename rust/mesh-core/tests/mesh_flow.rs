use mesh_core::{ApiAction, MeshCoreApi, decode_message};

#[test]
fn delivers_private_message_from_a_to_c_through_b() {
    let node_a = MeshCoreApi::new("node_a".to_string());
    let node_b = MeshCoreApi::new("node_b".to_string());
    let node_c = MeshCoreApi::new("node_c".to_string());
    ...
    let create_result = node_a.create_private_text_bytes(
        "node_c".to_string(),
        "Привет через B".to_string(),
        1710000000,
    );

    assert_eq!(create_result.error, None);

    let bytes_from_a = create_result.bytes;

    let b_actions = node_b.handle_incoming_bytes("node_a".to_string(), bytes_from_a);
    ...
    let c_actions = node_c.handle_incoming_bytes("node_b".to_string(), forwarded_bytes);
...
fn stores_message_on_b_and_delivers_later_when_c_becomes_available() {
    ...
    let pending_bytes = node_b.take_pending_bytes_for_peer("node_c".to_string());

    assert_eq!(pending_bytes.len(), 1);
    assert_eq!(node_b.pending_messages_count(), 0);

    let c_actions = node_c.handle_incoming_bytes("node_b".to_string(), pending_bytes[0].clone());
